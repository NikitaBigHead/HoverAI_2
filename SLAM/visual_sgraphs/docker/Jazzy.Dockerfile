...
RUN --mount=type=ssh git clone -b ros2-development https://github.com/IntelRealSense/realsense-ros.git
...
# USER root
# --- Install ROS 2 Jazzy packages (FIXED) ---
...
# Build the workspace
# --- Build the workspace (FIXED) ---
# --- Build the workspace (FINAL FIX) ---
...
RUN rosdep update && \
    ...
    --skip-keys 'librealsense2 librealsense2-dev launch_pytest tqdm python3-tqdm xacro realsense2_camera'"

# fix realsense version requirement
RUN sed -i 's/2.56.6/2.56.4/g' \
/home/$USERNAME/workspace/src/realsense-ros/realsense2_camera/CMakeLists.txt

RUN pip3 install setuptools==69.5.1

RUN /bin/bash -c "source /opt/ros/$ROS_DISTRO/setup.bash && \
    colcon build --symlink-install \
    --packages-skip realsense2_camera \
